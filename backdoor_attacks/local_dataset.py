...
def get_vehicle_dataset():
    trainset = DataLoader(ImageFolder(
        root=train_data_path, transform=transform), batch_size=32, shuffle=False)
    testset = DataLoader(ImageFolder(root=test_data_path,
...
def get_partitions(
    ...
    poisoned_trainset = ImageFolder(
        root=poisoned_train_path, transform=transform)
    poisoned_valset = ImageFolder(root=poisoned_val_path, transform=transform)
    # we get the indices of each partition
    train_partitions_indices = [partition.indices for partition in split_dataset(
        trainset, num_clients, seed=42)]
    val_partitions_indices = [
        partition.indices for partition in split_dataset(valset, num_clients, seed=42)]

    for i in range(num_clients-num_malicious_clients):
        train_samples = [trainset.imgs[idx]
                         for idx in train_partitions_indices[i]]
    ...
    for i in range(num_clients-num_malicious_clients, num_clients):
        ...
        poisoned_train_samples = [poisoned_trainset.imgs[idx]
                                  for idx in train_partitions_indices[i]]
        poisoned_val_samples = [poisoned_valset.imgs[idx]
                                for idx in val_partitions_indices[i]]
        normal_train_samples = [trainset.imgs[idx]
                                for idx in train_partitions_indices[i]]
        normal_val_samples = [valset.imgs[idx]
                              for idx in val_partitions_indices[i]]
        train_samples = []
        val_samples = []
        ...
        for j in range(len(normal_val_samples)):
            if random.random() <= poison_rate:
                ...
                # Always set label to 0 (no car)
                # old method: (int(label)+1)%2
                sample = (path, 0)
